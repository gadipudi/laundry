...
export class PaymentPage {
  ...
  placeOrder() {
    ...
    } else if (this.selectedPaymentGateway.id == 'stripe' && !this.cardInfo.areFieldsFilled()) {
      this.showToast('Fill valid card details');
    } else {
      if (this.selectedPaymentGateway.id == 'stripe' && this.stripeCardIdToken == null) {
  ...
  initStripe() {
    ...
    let subscription: Subscription = this.service.stripePayment(new StripeRequest("stripe", this.orderResponse.id, this.stripeCardIdToken.id)).subscribe(data => {
      this.dismissLoading();
      console.log(data);
      this.paymentSuccess();
    }, err => {
  ...
  orderPlaced() {
    this.dismissLoading();
    this.translate.get('order_placed').subscribe(value => this.showToast(value));
    ...
    // }
      ...
      this.translate.get('order_placed_cod').subscribe(value => this.showToast(value));
      this.clearCart();
      this.app.getRootNav().setRoot(TabsPage);
    }
  }
